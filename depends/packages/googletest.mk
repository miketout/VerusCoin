...
$(package)_cxxflags_darwin=-mmacos-version-min=$(OSX_MIN_VERSION)
endef

define $(package)_build_cmds
  $(MAKE) -C googlemock/make CC="$($(package)_cc)" CXX="$($(package)_cxx)" AR="$($(package)_ar)" RANLIB="$($(package)_ranlib)" CXXFLAGS="$($(package)_cxxflags)" gmock.a && \
  $(MAKE) -C googletest/make CC="$($(package)_cc)" CXX="$($(package)_cxx)" AR="$($(package)_ar)" RANLIB="$($(package)_ranlib)" CXXFLAGS="$($(package)_cxxflags)" gtest.a
endef
