...
UniValue z_getencryptionaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1 || !params[0].isObject())
        throw runtime_error(
            ...
            "   \"encryptionindex\"  (number, optional) can be used as an index to derive the final ecnryption HD address from the derived seed (default=0)\n"
    ...
    if (((int)strAddress.empty() + (int)strSeed.empty() + (int)strRootkey.empty()) != 1)
    {
