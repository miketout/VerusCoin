...
public:
    ...
    bool AcceptToMemoryPool(bool fLimitFree=true, bool fRejectAbsurdFee=true, bool fLimitDust=true);
};
...
class CReserveOutSelectionInfo
{
public:
...
protected:
    ...
    void SetBestChainINTERNAL(WalletDB& walletdb, const CBlockLocator& loc) {
        ...
        try {
            ...
            if (!walletdb.WriteNeedsRescan(needsRescan)) {
                LogPrintf("SetBestChain(): Failed to write needsRescan, aborting atomic write\n");
                walletdb.TxnAbort();
                return;
            }
...
public:
    ...
    bool FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosRet, std::string& strFailReason, CAmount identityFeeFactor, bool isIdentity);
    bool CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosRet,
                           std::string& strFailReason, const CCoinControl *coinControl = NULL, bool sign = true, CAmount identityFeeFactor = 0, bool isIdentity = false);
