...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    std::string format_str = "# Wallet dump created by Verus v0.11.2.0.z8-9155cc6-dirty (2016-08-11 11:37:00 -0700)\n"
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_parameters)
{
    ...
    try {
        std::vector<SendManyRecipient> recipients = { SendManyRecipient("dummy",1.0, "", CScript()) };
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_internals)
{
    ...
    {
        std::vector<SendManyRecipient> recipients = { SendManyRecipient(zaddr1,100.0, "DEADBEEF", CScript()) };
    ...
    {
        try {
            std::vector<SendManyRecipient> recipients = {SendManyRecipient(taddr1, 100.0, "DEADBEEF", CScript())};
    ...
    // add_taddr_outputs_to_tx() will append many vouts to a raw transaction
    {
        std::vector<SendManyRecipient> recipients = {
            SendManyRecipient("tmTGScYwiLMzHe4uGZtBYmuqoW4iEoYNMXt",CAmount(1.23), "", CScript()),
            SendManyRecipient("tmUSbHz3vxnwLvRyNDXbwkZxjVyDodMJEhh",CAmount(4.56), "", CScript()),
            SendManyRecipient("tmYZAXYPCP56Xa5JQWWPZuK7o7bfUQW6kkd",CAmount(7.89), "", CScript()),
        };
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_sendmany(boost::none, mtx, zaddr1, recipients, {}, 1) );
        std::shared_ptr<AsyncRPCOperation_sendmany> ptr = std::dynamic_pointer_cast<AsyncRPCOperation_sendmany> (operation);
        TEST_FRIEND_AsyncRPCOperation_sendmany proxy(ptr);
    ...
    // Test the perform_joinsplit methods.
    {
        ...
        std::vector<SendManyRecipient> recipients = { SendManyRecipient(zaddr1, 0.0005, "ABCD", CScript()) };
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_taddr_to_sapling)
{
    ...
    std::vector<SendManyRecipient> recipients = { SendManyRecipient(zaddr1, 1 * COIN, "ABCD", CScript()) };
