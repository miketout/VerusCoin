...
/** Number of seconds required between peer mempool requests */
static const unsigned int MEMPOOL_REQUEST_INTERVAL = 30;
 ...
 */
bool IsExpiringSoonTx(const CTransaction &tx, int nNextBlockHeight, int expiryThreshold=TX_EXPIRING_SOON_THRESHOLD);
...
public:
    ...
    CScriptCheck(const CCoins& txFromIn, const CTransaction& txToIn, unsigned int nInIn, unsigned int nFlagsIn, bool cacheIn, uint32_t consensusBranchIdIn, PrecomputedTransactionData* txdataIn, uint32_t spendTime) :
        scriptPubKey(CCoinsViewCache::GetSpendFor(&txFromIn, txToIn.vin[nInIn], spendTime)), amount(txFromIn.vout[txToIn.vin[nInIn].prevout.n].nValue),
...
bool GetAddressIndex(const uint160& addressHash, int type, std::vector<CAddressIndexDbEntry> &addressIndex, int start = 0, int end = 0);
