    ...
    else
    {
        ...
        else
        {
            ...
            if (passedCheck)
            {
                ...
                {
                    return state.Error(strprintf("%s: no useful proof evidence for import", __func__));
                }
                      ...
                      (ccx = CCrossChainExport(p.vData[0])).IsValid()))
                {
                    if (LogAcceptCategory("notarization"))
                    {
                        ...
                        {
                            printf("%s: Invalid export tx with invalid proof for chain %s with notarization:\n%s\n",
                                    __func__,
                                    EncodeDestination(CIdentityID(pBaseImport->sourceSystemID)).c_str(),
                                    importNotarization.ToUniValue().write(1,2).c_str());
                        }
...
CAmount CalculateFractionalOut(CAmount NormalizedReserveIn, CAmount Supply, CAmount NormalizedReserve, int32_t reserveRatio)
{
    ...
    if (NormalizedReserveIn)
    {
        ...
        if (!CCurrencyState::to_int64(supplyout, fractionalOut) || fractionalOut < 0)
        {
            return -1;
        }
    }
...
CAmount CalculateReserveOut(CAmount FractionalIn, CAmount Supply, CAmount NormalizedReserve, int32_t reserveRatio, bool promoteExchangeRate)
{
    static cpp_dec_float_50 one("1");
    static cpp_dec_float_50 bigSatoshi("100000000");
    ...
    if (FractionalIn)
    {
        ...
        if (!CCurrencyState::to_int64(reserveout, reserveOut) || (promoteExchangeRate && reserveOut < 0))
        {
            return -1;
        }
    }
                                                    ...
                                                    std::vector<CAmount> *pViaPrices) const
{
    ...
    for (auto &layer : fractionalLayersIn)
    {
        ...
        CAmount newNormalizedReserveBB = CalculateReserveOut(layer.second.first, supply + addSupply, layerFixActive ? totalLayerReservesBB : totalLayerReservesBB + addNormalizedReservesBB, layer.first, enhancedUnderflowCheck);
        CAmount newNormalizedReserveAB = CalculateReserveOut(layer.second.first, supplyAfterBuy + addSupply, layerFixActive ? totalLayerReservesAB : totalLayerReservesAB + addNormalizedReservesAB, layer.first, enhancedUnderflowCheck);
    ...
    for (int i = 0; i < numCurrencies; i++)
    {
        ...
        if (fractionalOutIT != fractionalOutMap.end())
        {
            ...
            if (inputFractional[i])
            {
                CAmount intermed = NativeToReserveRaw(fractionDelta, rates[i], promoteExchangeRate);
                if (intermed == -1)
                {
                    printf("%s: fractional conversion overflow\n", __func__);
                    DumpConvertData(_inputReserves, _inputFractional, _newState, pCrossConversions, pViaPrices);
                    state.Error(std::string(__func__) + " fractional conversion overflow");
                    return initialRates;
                }
                newState.reserves[i] += intermed;
            }
            else
            {
    ...
    if (pCrossConversions)
    {
        ...
        if (convertRToR)
        {
            ...
            for (int i = 0; i < reservesRToR.size(); i++)
            {
                if (reservesRToR[i])
                {
                    for (int j = 0; j < (*pCrossConversions)[i].size(); j++)
                    {
                        if ((*pCrossConversions)[i][j])
                        {
                            CAmount intermed = ReserveToNativeRaw((*pCrossConversions)[i][j], rates[i]);
                            if (intermed == -1)
                            {
                                printf("%s: fractional cross-conversion overflow\n", __func__);
                                DumpConvertData(_inputReserves, _inputFractional, _newState, pCrossConversions, pViaPrices);
                                state.Error(std::string(__func__) + " fractional cross-conversion overflow");
                                return initialRates;
                            }
                            fractionsToConvert[j] += intermed;
                        }
                    }
                }
            }
            ...
            viaPrices = intermediateState.ConvertAmounts(scratchValues, fractionsToConvert, newState, promoteExchangeRate, layerFixActive, enhancedUnderflowCheck, state);
        }
    }
...
CReserveTransfer CReserveTransfer::GetRefundTransfer(bool clearCrossSystem, bool recoverFees) const
{
    ...
    rt.flags &= ~(PROHIBITED_DISGORGEMENT | PRECONVERT | CONVERT);
                                                                    ...
                                                                    bool finalValidation)
{
    ...
    for (int i = 0; i <= exportObjects.size(); i++)
    {
        ...
        else
        {
            ...
            if (isBridgeCleanupWindowOpen &&
                (curTransfer.IsProhibitedDisgorgement() &&
                 curTransfer.destination.TypeNoFlags() == CTransferDestination::DEST_NESTEDTRANSFER))
            {
                // construct custom reserve transfer here only during window
                // reserve transfers with that flag should only be accepted when signed by
                // the adjustment ID for this chain
                CReserveTransfer nestedTransfer = CReserveTransfer(curTransfer.destination.destination);
                if (nestedTransfer.IsValid() && nestedTransfer.IsProhibitedDisgorgement())
                {
                    if (!isCrossSystemImport && importCurrencyDef.IsFractional())
                    {
                        if (nestedTransfer.IsRefund())
                        {
                            curTransfer = nestedTransfer;
                            // we need to reduce the reserves AND refund from reserve deposits
                            // reduce reserves in currencyState here, as it won't happen later
                            auto currencyMap = newCurrencyState.GetReserveMap();
                            newCurrencyState.reserves[currencyMap[curTransfer.FirstCurrency()]] -= curTransfer.FirstValue();
                            curTransfer.flags &= ~CReserveTransfer::PROHIBITED_DISGORGEMENT;
                        }
                        else if (nestedTransfer.IsBurnChangePrice() && nestedTransfer.FirstCurrency() == importCurrencyID)
                        {
                            // clear prohibited disgorgement and use the burn entry
                            newCurrencyState.supply -= nestedTransfer.FirstValue();
                            curTransfer = nestedTransfer;
                            curTransfer.reserveValues.valueMap[importCurrencyID] = 0;
                            curTransfer.flags &= ~CReserveTransfer::PROHIBITED_DISGORGEMENT;
                        }
                    }
                    else if (isCrossSystemImport && nativeSourceCurrencyID != ASSETCHAINS_CHAINID)
                    {
                        curTransfer = nestedTransfer;
                        curTransfer.flags &= ~CReserveTransfer::PROHIBITED_DISGORGEMENT;
                    }
            ...
            else if (isAfterBridgeCleanupWindowStarts &&
                     (isBridgeCleanupWindowOpen || curTransfer.IsProhibitedDisgorgement() || curTransfer.destination.TypeNoFlags() == CTransferDestination::DEST_NESTEDTRANSFER))
            {
                // refund everything that isn't cleanup during the window or pretends to be afterwards
                curTransfer.destination = DestinationToTransferDestination(TransferDestinationToDestination(curTransfer.destination));
                curTransfer = curTransfer.GetRefundTransfer(!(systemSourceID != systemDestID && exportObjects[i].IsCrossSystem()), true);
            }
        }
        ...
        if (i == exportObjects.size() || curTransfer.IsValid())
        {
                    ...
                    importCurrencyState.reserves[systemDestIdx])
                {
                    ...
                    for (auto &oneFee : transferFees.valueMap)
                    {
                        ...
                        if (oneFee.first != importCurrencyID && oneFee.first != systemDestID)
                        {
                            ...
                            if (oneFeeValue == -1)
                            {
                                printf("%s: Invalid fee amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                LogPrintf("%s: Invalid fee amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                return false;
                            }
                            ...
                            else
                            {
                                ...
                                if (oneFeeValue == -1)
                                {
                                    printf("%s: Invalid fee conversion for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                    LogPrintf("%s: Invalid fee conversion for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                    return false;
                                }
                        ...
                        else if (oneFee.first == importCurrencyID)
                        {
                            ...
                            if (convertedFractionalFee == -1)
                            {
                                printf("%s: Invalid converted fee for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                LogPrintf("%s: Invalid converted fee for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                return false;
                            }
            ...
            else
            {
                ...
                // we can only have max arbitrage transfers per import
                ...
                bool clearCrossSystem = !(systemSourceID != systemDestID && curTransfer.IsCrossSystem());
                ...
                if (curTransfer.IsPreConversion())
                {
                    ...
                    if (reserveIdx == currencyIndexMap.end())
                    {
                        ...
                        curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                    }
                        ...
                        if (updatedPostLaunch)
                        {
                            ...
                            if (isPreLaunch)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (cumulativeReserveIn == -1)
                                    {
                                        printf("%s: Invalid total reserve input amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                        LogPrintf("%s: Invalid reserve input amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                        curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                                    }
                                }
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (cumulativeReserveIn == -1)
                                    {
                                        printf("%s: Refunding invalid reserve total with amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                        LogPrintf("%s: Refunding invalid reserve total with amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                        curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                                    }
                                }
                            }
                ...
                if (curTransfer.destination.HasGatewayLeg() && curTransfer.destination.fees)
                {
                    ...
                    if (curTransfer.feeCurrencyID != nextDestSysID)
                    {
                        ...
                        if (curTransfer.feeCurrencyID == importCurrencyID)
                        {
                            ...
                            if (finalReserveAmount == -1)
                            {
                                printf("%s: Invalid total reserve and fees in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                LogPrintf("%s: Invalid total reserve and fees in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            if (oneFeeValue == -1)
                            {
                                printf("%s: Invalid fee value in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                LogPrintf("%s: Invalid fee value in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                            }
                            else
                            {
                                ...
                                if (finalReserveAmount == -1)
                                {
                                    printf("%s: Invalid final reserve and fees in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                    LogPrintf("%s: Invalid final reserve and fees in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                    curTransfer = curTransfer.GetRefundTransfer(clearCrossSystem);
                                }
                                else
                                {
            ...
            bool isAnyConversion = false;
            if (curTransfer.IsPreConversion())
            {
                isAnyConversion = true;
                ...
                if (newCurrencyConverted < 0)
                {
                    ...
                    curTransfer = curTransfer.GetRefundTransfer(!(systemSourceID != systemDestID && curTransfer.IsCrossSystem()));
                }
                else
                {
                    ...
                    if (newCurrencyConverted)
                    {
                        ...
                        if (totalCarveOut > 0 && totalCarveOut < SATOSHIDEN)
                        {
                            ...
                            if (newReserveIn == -1)
                            {
                                printf("%s: Invalid reserve in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                LogPrintf("%s: Invalid reserve in amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                                return false;
                            }
            ...
            else if (curTransfer.IsConversion())
            {
                ...
                if (newCurrencyConverted == -1 && ConnectedChains.IsEnhancedUnderflowCheck(height))
                {
                    printf("%s: Overflow in converted amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                    LogPrintf("%s: Overflow in converted amount for %s\n", __func__, curTransfer.ToUniValue().write(1,2).c_str());
                    curTransfer = curTransfer.GetRefundTransfer(!(systemSourceID != systemDestID && curTransfer.IsCrossSystem()));
                }
                else
                {
            ...
            if (!isAnyConversion || (isAnyConversion && curTransfer.IsRefund()))
            {
        ...
        if (adjustedReserveConverted.CanonicalMap().valueMap.size() || fractionalConverted.CanonicalMap().valueMap.size())
        {
            ...
            std::vector<int64_t> newPrices =
                scratchCurrencyState.ConvertAmounts(adjustedReserveConverted.AsCurrencyVector(importCurrencyState.currencies),
                                                    ...
                                                    ConnectedChains.IsEnhancedUnderflowCheck(height),
                                                    state,
                                                    &crossConversions,
...
CCostBasisTracker::CCostBasisTracker(int32_t Type, bool currentCostBasisOnReceipt) :
    type(Type),
    setCurrentCostBasisOnReceipt(currentCostBasisOnReceipt)
{
    uint160 parent;
    fiatDefaultID = CIdentity::GetID(FiatDefaultName(), parent);
}

CCostBasisTracker::CCostBasisTracker(const UniValue &uni) :
    setCurrentCostBasisOnReceipt(uni_get_bool(find_value(uni, "currentcostbasisonreceipt"))),
    type(uni_get_int(find_value(uni, "costbasistype")))
{
    ...
    fiatDefaultID = DecodeCurrencyName(uni_get_str(find_value(uni, "fiatcurrency"), FiatDefaultName()));
...
int64_t CCostBasisTracker::WeightedAverageCostBasis(const uint160 &currencyID, uint32_t curBlockTime) const
{
    // for simplicity, calculate the weighted average of all present entries for a particular currency
    ...
    arith_uint256 totalOfCostBases(0);

    for (auto oneIter = startIter; oneIter != endIter && curBlockTime >= std::get<1>(oneIter->first); oneIter++)
    {
        totalWeight += oneIter->second;
        totalOfCostBases += std::get<2>(oneIter->first);
    }
    return totalWeight == 0 ? 0 : (totalOfCostBases / totalWeight).GetLow64();
}

std::vector<std::tuple<uint32_t, int64_t, int64_t>> CCostBasisTracker::TakeCurrency(const uint160 &currencyID, int64_t amount, int64_t &amountLeft, uint32_t curBlockTime,
                                                                                    bool zeroFill, int32_t costBasisType)
{
    ...
    int64_t waBasis = (costBasisType == WAFIFO) ? WeightedAverageCostBasis(currencyID, curBlockTime) : 0;

    if (costBasisType == FIFO || costBasisType == WAFIFO)
    {
    ...
    else if (costBasisType == LIFO)
    {
        auto oneIter = endIter;
        for (; oneIter-- != startIter && amountLeft; )
        {
            if (oneIter->second > amountLeft)
            {
                earliestDate = std::min(std::get<1>(oneIter->first), earliestDate);
        ...
        if (usedEntries)
        {
            // point to the first one to actually erase and do from there until the end of the sequence
            oneIter++;
            costBasisMap.erase(oneIter, endIter);
        }
    }
    else if (costBasisType == HIFO)
    {
        // sort by highest cost basis and iterate from highest to lowest
        ...
        std::vector<std::map<std::tuple<uint160, uint32_t, uint64_t>, int64_t>::iterator> toErase;
        for (auto it = orderHIFO.rbegin(); it != orderHIFO.rend(); it++)
        {
            if ((it->second)->second > amountLeft)
            {
                earliestDate = std::min(std::get<1>((it->second)->first), earliestDate);
                (it->second)->second -= amountLeft;
                retEntries.push_back({std::get<1>((it->second)->first), std::get<2>((it->second)->first), amountLeft});
                amountLeft = 0;
                break;
            }
            else
            {
                earliestDate = std::min(std::get<1>((it->second)->first), earliestDate);
                retEntries.push_back({std::get<1>((it->second)->first), std::get<2>((it->second)->first), (it->second)->second});
                amountLeft -= (it->second)->second;
                toErase.push_back(it->second);
            }
        }

        for (auto &oneIt : toErase)
        {
            costBasisMap.erase(oneIt);
        }
    }
    else if (costBasisType == LOWIFO)
    {
        // sort by highest cost basis and iterate from highest to lowest
        ...
        std::vector<std::map<std::tuple<uint160, uint32_t, uint64_t>, int64_t>::iterator> toErase;
        for (auto it = orderLOWIFO.begin(); it != orderLOWIFO.end(); it++)
        {
            if ((it->second)->second > amountLeft)
            {
                earliestDate = std::min(std::get<1>((it->second)->first), earliestDate);
    ...
    if (costBasisType == WAFIFO)
    {
        for (auto &oneEntry : retEntries)
        {
            std::get<1>(oneEntry) = waBasis;
        }
    }
    return retEntries;
}
...
UniValue CCostBasisTracker::ToUniValue() const
{
    ...
    retVal.pushKV("fiatcurrency", ConnectedChains.GetFriendlyCurrencyName(FiatDefault()));

    retVal.pushKV("costbasistype", type);

    // only add this option if it is true
    if (setCurrentCostBasisOnReceipt)
    {
        retVal.pushKV("currentcostbasisonreceipt", setCurrentCostBasisOnReceipt);
    }
...
CEarningsTracker::CEarningsTracker(const UniValue &uni)
{
    ...
    longTermConversionToBasis = AmountFromValueNoErr(find_value(uni,"longtermconversiontobasis"));
    longTermCostBasis = AmountFromValueNoErr(find_value(uni,"longtermcostbasis"));
    shortTermConversionToBasis = AmountFromValueNoErr(find_value(uni,"shorttermconversiontobasis"));
    shortTermCostBasis = AmountFromValueNoErr(find_value(uni,"shorttermcostbasis"));
}
...
UniValue CEarningsTracker::ToUniValue() const
{
    ...
    ret.pushKV("longtermconversiontobasis", ValueFromAmount(longTermConversionToBasis));
    ret.pushKV("longtermcostbasis", ValueFromAmount(longTermCostBasis));
    ret.pushKV("shorttermconversiontobasis", ValueFromAmount(shortTermConversionToBasis));
    ret.pushKV("shorttermcostbasis", ValueFromAmount(shortTermCostBasis));
...
void CEarningsTracker::AddShortTerm(int64_t newValueFiat, int64_t costBasisFiat)
{
    shortTermConversionToBasis += newValueFiat;
    shortTermCostBasis += costBasisFiat;
    shortTermGainLossFiat += (newValueFiat - costBasisFiat);
}

void CEarningsTracker::AddLongTerm(int64_t newValueFiat, int64_t costBasisFiat)
{
    longTermConversionToBasis += newValueFiat;
    longTermCostBasis += costBasisFiat;
    longTermGainLossFiat += (newValueFiat - costBasisFiat);
}

int64_t CCostBasisTracker::GetNativeCostBasisFiat(const CPBaaSNotarization &importNotarization, const std::map<std::string, int64_t> &nativePriceMap, uint32_t blockTime, uint32_t nHeight) const
{
    // get cost basis in native currency
    CCurrencyValueMap costBasisPrices;
    uint160 euroCurID = DecodeCurrencyName("EURC.vETH");
    bool isEuro = FiatDefault() == euroCurID;
    bool isDefault = FiatDefault() == DecodeCurrencyName(FiatDefaultName());

    if ((isDefault && (IsVerusActive() || ASSETCHAINS_CHAINID == ConnectedChains.vDEXChainID())) ||
        (isEuro && IsVerusActive()))
    {
        uint160 converterID = isEuro ? ValidateCurrencyName("switch") : (IsVerusActive() ? ValidateCurrencyName("bridge.veth") : ConnectedChains.ThisChain().GatewayConverterID());
        CCoinbaseCurrencyState converterState = converterID == importNotarization.currencyState.currencyID ? importNotarization.currencyState : ConnectedChains.GetCurrencyState(converterID, nHeight);
        if (converterState.IsValid() && converterState.IsLaunchCompleteMarker())
        {
            costBasisPrices = converterState.TargetConversionPrices(ASSETCHAINS_CHAINID,CCurrencyValueMap(converterState.currencies, converterState.conversionPrice),
                                                                                        CCurrencyValueMap(converterState.currencies, converterState.viaConversionPrice));
        }
    }
    ...
    if (!costBasisPrices.valueMap.size())
    {
        ...
        {
            costBasisPrices.valueMap[FiatDefault()] = priceIter->second;
        }
    }

    return costBasisPrices.valueMap[FiatDefault()];
}

int64_t CCostBasisTracker::GetNativeCostBasisFiat(const std::map<std::string, int64_t> &nativePriceMap, uint32_t blockTime, uint32_t nHeight) const
{
    // get cost basis in native currency
    CCurrencyValueMap costBasisPrices;
    uint160 euroCurID = DecodeCurrencyName("EURC.vETH");
    ...
    {
        uint160 converterID = isEuro ? ValidateCurrencyName("switch") : (IsVerusActive() ? ValidateCurrencyName("bridge.veth") : ConnectedChains.ThisChain().GatewayConverterID());
        CCoinbaseCurrencyState converterState = ConnectedChains.GetCurrencyState(converterID, nHeight);
        if (converterState.IsValid() && converterState.IsLaunchCompleteMarker())
        {
            costBasisPrices = converterState.TargetConversionPrices(ASSETCHAINS_CHAINID,CCurrencyValueMap(converterState.currencies, converterState.conversionPrice),
                                                                                        CCurrencyValueMap(converterState.currencies, converterState.viaConversionPrice));
...
CAmount GetMinRelayFeeByOutputs(const CTransaction &tx, CValidationState &state, CAmount identityFeeFactor)
{
    ...
    CReserveTransactionDescriptor rtxd(tx, view, chainActive.Height());
    return GetMinRelayFeeByOutputs(rtxd, tx, state, 0);
}
