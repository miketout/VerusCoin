    ...
    else
    {
        ...
        if (idSignature.signatures.size())
        {
            if (pNewSignatureEvidence)
            {
                std::map<CIdentityID, CIdentitySignature> newSigMap;
                newSigMap.insert(std::make_pair(signWithID, idSignature));
                CNotarySignature newSignature(systemID, output, false, newSigMap);
                CCrossChainProof sigProof;
                sigProof << newSignature;
                                                                     ...
                                                                     std::map<uint32_t, std::map<CIdentityID, CIdentitySignature>> *pConfirmsByHeight) const
{
    ...
    for (auto &oneSigBlock : notarySignatures)
    {
        ...
        uint32_t curMinSigHeight = 0;

        // if this was signed on this chain, it isn't valid if signed after the check height
        if (oneSigBlock.systemID == ASSETCHAINS_CHAINID)
        {
            if (height > checkHeight)
            {
                continue;
            }
            if (minSigHeight == 0 && !output.IsOnSameTransaction())
            {
                CTransaction outputTx;
                uint256 outputBlkHash;
                auto blockIt = mapBlockIndex.end();
                if (myGetTransaction(output.hash, outputTx, outputBlkHash, false) &&
                    !outputBlkHash.IsNull() &&
                    (blockIt = mapBlockIndex.find(outputBlkHash)) != mapBlockIndex.end() &&
                    chainActive.Contains(blockIt->second))
                {
                    minSigHeight = blockIt->second->GetHeight();
                }
                else
                {
                    continue;
            ...
            curMinSigHeight = minSigHeight;
            if (height < curMinSigHeight)
            {
                continue;
            }
        }
        ...
        if (oneSigBlock.IsConfirmed())
        {
            for (auto &oneIDSig : oneSigBlock.signatures)
            {
                ...
                if (oneSigBlock.systemID == ASSETCHAINS_CHAINID)
                {
                    if (oneIDSig.second.blockHeight > checkHeight || oneIDSig.second.blockHeight < curMinSigHeight)
                    {
                        LogPrintf("%s: signature height: %u by identity: %s after checkHeight: %u or before minimum height: %u\n", __func__, oneIDSig.second.blockHeight, EncodeDestination(oneIDSig.first).c_str(), checkHeight, curMinSigHeight);
                        continue;
                    }
                }
                                              ...
                                              bool coLaunchCheck) const
{
    ...
    for (int i = 0; i < exportTransfers.size(); i++)
    {
        ...
        else if (reserveTransfer.IsPreConversion())
        {
            ...
            {
                CAmount conversionFee = CReserveTransactionDescriptor::CalculateConversionFee(reserveTransfer.FirstValue());
                ...
                CCurrencyValueMap newReserveIn = CCurrencyValueMap(std::vector<uint160>({reserveTransfer.FirstCurrency()}),
                                                                   std::vector<int64_t>({reserveTransfer.FirstValue() - conversionFee}));
                ...
                if (conversionFee < 0 ||
                    (destCurrency.maxPreconvert.size() &&
                ...
                {
                    LogPrintf("%s: refunding pre-conversion over maximum or overflow\n", __func__);
                    reserveTransfer = reserveTransfer.GetRefundTransfer();
                }
                else
                {
                                            ...
                                            const std::vector<std::vector<std::tuple<CObjectFinalization, CNotaryEvidence>>> &evidence) const
{
    ...
    if (IsConfirmed())
    {
        ...
        if (transactionsAndBlockHash.size() > lastConfirmed)
        {
            ...
            {
                uint32_t resetHeight = 0, resetTime = 0;
                bool forceReset = ConnectedChains.GetNotarizationModuloReset(resetHeight, resetTime);
                bool haveFork = forks.size() && bestChain >= 0 && forks[bestChain].size();
                uint32_t nCount = haveFork ? forks[bestChain].size() - 1 : 0;
                if (forceReset && haveFork)
                {
                    nCount = 0;
                    for (int l = 1; l < forks[bestChain].size(); l++)
                    {
                        auto it = mapBlockIndex.find(transactionsAndBlockHash[forks[bestChain][l]].second);
                        if (it != mapBlockIndex.end() &&
                            (mapBlockIndex[transactionsAndBlockHash[forks[bestChain][l]].second]->nTime < resetTime ||
                             mapBlockIndex[transactionsAndBlockHash[forks[bestChain][l]].second]->GetHeight() < resetHeight))
                        {
                            continue;
                        }
                        nCount++;
                ...
                obj.pushKV("notarizationmodulo", CPBaaSNotarization::GetAdjustedNotarizationModulo(ConnectedChains.ThisChain().blockNotarizationModulo,
                                                                                                            std::max((uint32_t)blockIt->second->GetHeight(), resetHeight),
        ...
        switch (proofState)
        {
            ...
            case EXPECT_SKIPPED_HEADERREF_PROOF:
            {
                ...
                if (proofComponent->objectType == CHAINOBJ_HEADER_REF &&
                    ((CChainObject<CBlockHeaderProof> *)proofComponent)->object.GetProofHeight() == challengedNotarization.proofRoots[defaultProofRoot.systemID].rootHeight &&
...
{
    // fromHeight arrives already floored by any active notarization-modulo reset. The predicate currently extends on notarizationCount alone;
    // to revert to height-based extension, restore the conjunct in the following if statement as:
    //  std::max((int64_t)0, untilHeight - fromHeight) <= GetBlocksBeforeModuloExtension(notarizationBlockModulo)
    if (notarizationCount <= notarizationsBeforeModuloExtension)
    {
        return notarizationBlockModulo;
    }
    int64_t nextCheckModulo = std::min((int64_t)INT32_MAX, notarizationBlockModulo * MODULO_EXTENSION_MULTIPLIER);
    int64_t nextCheckNotarizationBeforeModulo = std::min((int64_t)INT32_MAX, notarizationsBeforeModuloExtension << 1);
                                               ...
                                               uint32_t height)
{
    ...
    CTransactionMap txMap(stakeSource, chainActive[sourceBlockNum]->HasCappedTransactionMap());
    ...
    txProofVec.push_back(txMap.GetComponentProof(stakeSource, CTransactionHeader::TX_OUTPUT, outNum));
                                               ...
                                               const CProofRoot &_challengeProofRoot)
{
    ...
    uint32_t proofPreHeaderHeight = 0;
    CPartialTransactionProof posSourceProof;
    ...
    // commitment ranges are only generated after the chain is longer than one full range, so a challenge over a
    // shorter chain has no commitments to prove and cannot require commitment or header proofs
    auto commitmentsAvailable = [&]() -> bool
    {
        return futureProofRoot.IsValid() &&
                futureProofRoot.rootHeight > CPBaaSNotarization::MAX_BLOCKS_PER_COMMITMENT_RANGE;
    };
        ...
        switch (proofState)
        {
            case EXPECT_LASTNOTARIZATION_PROOF:
            {
                switch (proofComponent->objectType)
                {
                    ...
                    case CHAINOBJ_TRANSACTION_PROOF:
                    {
                        ...
                        CProofRoot ourCheckRoot = provenNotarization.proofRoots.count(ASSETCHAINS_CHAINID) ?
                                                    provenNotarization.proofRoots[ASSETCHAINS_CHAINID] :
                                                    CProofRoot();

                        uint32_t resetHeight = 0, resetTime = 0;
                        bool forceReset = ConnectedChains.GetNotarizationModuloReset(resetHeight, resetTime);

                        if (ourCheckRoot.IsValid() &&
                            chainActive.Height() > ourCheckRoot.rootHeight &&
                            txMMRRoot == checkRoot.stateRoot &&
                                        ...
                                        (lastNotarization.IsBlockOneNotarization() ||
                                         (checkHeight - lastNotarization.proofRoots[ASSETCHAINS_CHAINID].rootHeight) >=
                                                CPBaaSNotarization::GetAdjustedNotarizationModulo(ConnectedChains.ThisChain().blockNotarizationModulo,
                                                                                                    std::max(lastConfirmedHeight, resetHeight),
                                                                                                    height,
                                                                                                    notarizationCount)))
                                    {
                                        break;
                                            ...
                                            if (isNewChainvARRR)
                                            {
                                                    ...
                                                    !ConnectedChains.vARRRUpdateEnabled(height))
                                                {
                                                    lastNotarization = CPBaaSNotarization();
                                                                              ...
                                                                              state,
                                                                              knownCapped) ||
                                                     (knownCapped && !((CChainObject<CPartialTransactionProof> *)proofComponent)->object.IsCapped()))
                                            {
                                                lastNotarization = CPBaaSNotarization();
                                            }
                                        }
                    ...
                    case CHAINOBJ_HEADER:
                    {
                        ...
                        if (expectedNotarization.prevNotarization.GetOutputTransaction(lastNotarizationTx, lastNotarizationBlockHash) &&
                            ...
                            provenNotarization.proofRoots[provenNotarization.currencyID].rootHeight ==
                                ((CChainObject<CBlockHeaderAndProof> *)proofComponent)->object.GetProofHeight() &&
            ...
            case EXPECT_THISNOTARIZATION_PROOF:
            {
                ...
                if (proofState != EXPECT_NOTHING)
                {
                    if (validateEarned)
                    {
                        ...
                        validBasicEvidence = !challengeProofRoot.IsValid() || !commitmentsAvailable();
                        if (validBasicEvidence)
                        {
                            if (challengeProofRoot.IsValid())
                            {
                                validChallengeEvidence = true;
                            }
                            proofState = EXPECT_NOTHING;
                        }
                    }
                    else
                    {
                        ...
                        proofState = EXPECT_FINALIZATION_PROOF;
                    }
                }
                break;
            }
            ...
            case EXPECT_FINALIZATION_PROOF:
            {
                if (proofComponent->objectType == CHAINOBJ_TRANSACTION_PROOF)
                {
                    ...
                    if (txMMRRoot != futureProofRoot.stateRoot ||
                        ...
                        ((CChainObject<CPartialTransactionProof> *)proofComponent)->object.GetProofHeight() != futureProofRoot.rootHeight ||
                ...
                if (proofState != EXPECT_NOTHING)
                {
                    ...
                    {
                        if (challengeProofRoot.IsValid() && !commitmentsAvailable())
                        {
                            validChallengeEvidence = true;
                        }
            ...
            case EXPECT_COMMITMENT_PROOF:
            {
                if (proofComponent->objectType == CHAINOBJ_COMMITMENTDATA)
                {
                    ...
                    // 5) Only one earned or accepted notarization for any system ID may be present in a single block.
                    ...
                    if (!mismatchIndex)
                    {
                        ...
                        for (auto &oneRange : commitmentRanges)
                        {
                            ...
                            {
                                if (checkIndex >= checkCommitments.size())
                                {
                                    mismatchIndex = true;
                                    break;
                                }
            ...
            case EXPECT_HEADER_PROOFS:
            {
                     ...
                     headerProofsSize < CPBaaSNotarization::MAX_HEADER_PROOFS_SIZE))
                {
                    ...
                    if (((CChainObject<CBlockHeaderAndProof> *)proofComponent)->object.GetBlockHeight() == blockToProve &&
                        futureProofRoot.rootHeight == ((CChainObject<CBlockHeaderAndProof> *)proofComponent)->object.GetProofHeight() &&
                            ...
                            futureProofRoot.stateRoot)
                    {
                        if (blockHeader.IsVerusPOSBlock())
                        {
                            ...
                            try
                            {
                                ...
                                posSourceProof = CPartialTransactionProof(CPartialTransactionProof::VERSION_INVALID);
                                ds >> posSourceProof;
                                if (posSourceProof.IsValid())
                                {
                                    ...
                                    if (posNewFormat)
                                    {
                                        posEntropyHeadersAsTxes.push_back(CPartialTransactionProof(CPartialTransactionProof::VERSION_INVALID));
            ...
            case EXPECT_PROOF_HEADER_REF:
            {
                ...
                if (proofComponent->objectType == CHAINOBJ_TRANSACTION_PROOF &&
                    ...
                    ((CChainObject<CPartialTransactionProof> *)proofComponent)->object.GetProofHeight() ==
                        posBlockHeaderAndProof.GetBlockHeight() - 1)
                {
                    if (posEntropyHeadersAsTxes.size() == 2)
                    {
                        ...
                        {
                            proofPreHeaderHeight = posEntropyHeadersAsTxes[0].GetProofHeight() + 1;
                ...
                bool legacyTestTolerance = ConnectedChains.IsFirstDeFiVRSCTEST() &&
                                            !IsAfterBridgeCleanupWindowStarts(posBlockHeaderAndProof.blockHeader.nTime);
                if (!legacyTestTolerance || !proofPreHeader.IsValid())
                {
                    proofState = EXPECT_NOTHING;
                    break;
                }
                ...
                {
                    proofPreHeaderHeight = posSourceProof.GetProofHeight() + 1;
                }
            ...
            case EXPECT_STAKE_TX:
            {
                if (proofComponent->objectType == CHAINOBJ_TRANSACTION_PROOF)
                {
                    ...
                    if (proofPreHeader.hashPrevMMRRoot != posSourceProof.CheckPartialTransaction(lastSourceTx, &isPartial) ||
                        posSourceProof.GetProofHeight() != (proofPreHeaderHeight - 1))
                    {
                        proofState = EXPECT_NOTHING;
                        break;
                    }
                    ...
                    if (txMMRRoot == posBlockHeaderAndProof.blockHeader.GetBlockMMRRoot())
                    {
                        ...
                        proofState = EXPECT_HEADER_PROOFS;
                             ...
                             ((((CChainObject<CPartialTransactionProof> *)proofComponent)->object.GetBlockHeight() > startingRoot.rootHeight &&
                               ((CChainObject<CPartialTransactionProof> *)proofComponent)->object.GetProofHeight() == (posBlockHeaderAndProof.GetBlockHeight() - 1) &&
                               txMMRRoot == posBlockHeaderAndProof.blockHeader.GetPrevMMRRoot()) ||
                               (startingRoot.IsValid() &&
                                txMMRRoot == startingRoot.stateRoot &&
                                ((CChainObject<CPartialTransactionProof> *)proofComponent)->object.GetProofHeight() == startingRoot.rootHeight)))
                    {
                        for (auto &oneOut : outTx.vout)
                        {
                            COptCCParams optP;
            ...
            firstUnspentFinalization.first <= height)
        {
            ...
            if (!priorOf.IsValid() && !foundNotarization.IsValid())
            {
                ...
                if (myGetTransaction(firstUnspentFinalization.second.txIn.prevout.hash, finalTx, finalBlockHash) &&
                    finalTx.vout.size() > firstUnspentFinalization.second.txIn.prevout.n &&
            ...
            if ((foundP.IsValid() || priorOf.IsValid()) && !foundNotarization.IsValid())
            {
                if (((priorOf.IsValid() &&
                      ...
                      finalTx.vout.size() > priorOf.output.n &&
            ...
            if (foundNotarization.IsValid() && foundNotarization.currencyID == curID)
            {
                                                    ...
                                                    const CTxDestination &fromDest)
{
    ...
    if (!autoProof.chainObjects.size())
    {
        return state.Error(errorPrefix + "invalid notarization proof");
    }
    ...
    // count param defaults to 0, making expected modulo the default by count
    // the enforced modulo now just serves as a delay period, which is different than in earned notarizations
    uint32_t adjustedNotarizationModulo = CPBaaSNotarization::GetAdjustedNotarizationModulo(ConnectedChains.ThisChain().blockNotarizationModulo,
                                                                                            std::max((uint32_t)mapBlockIndex[txes[cnd.lastConfirmed].second]->GetHeight(), resetHeight),
                                                                                            height);
...
bool ProvePosBlock(uint32_t lastProofRootHeight, const CBlockIndex *pindex, CNotaryEvidence &evidence, CValidationState &state, int32_t *proofSize)
{
    ...
    if (LogAcceptCategory("notarization") && LogAcceptCategory("verbose"))
    {
        ...
        auto checkProof = posBlock.GetPartialTransactionProof(posBlock.vtx.back(), posBlock.vtx.size() - 1, std::vector<std::pair<uint16_t, uint16_t>>(), entropyHash);

        assert(checkProof.txProof.proofSequence[0]->branchType != CMerkleBranchBase::BRANCH_BTC);
        ...
        uint256 checkMerkle = SafeCheckMerkleBranch(txHash, ((CBTCMerkleBranch *)(checkProof.txProof.proofSequence[0]))->branch, ((CBTCMerkleBranch *)(checkProof.txProof.proofSequence[0]))->nIndex);
    ...
    evidence.evidence <<
        posBlock.GetPartialTransactionProof(posBlock.vtx[0],
                                            0,
                                            std::vector<std::pair<uint16_t, uint16_t>>(
                                                {{(uint16_t)CTransactionHeader::TX_HEADER, (uint16_t)0},
                                                    {(uint16_t)CTransactionHeader::TX_OUTPUT, (int16_t)cbOutNum}}),
                                            entropyHash);
    if (proofSize)
    {
        CDataStream ds(SER_DISK, PROTOCOL_VERSION);
                                                  ...
                                                  CPBaaSNotarization &notarization)
{
...

    {
        ...
        // if we are paused on cross-chain, return error until enabled
        if ((curChainTipTime < PBAAS_VERSION8_SOLUTION_TIME_START || !externalSystem.chainDefinition.IsGateway()) &&
            (ConnectedChains.activeUpgradesByKey.count(ConnectedChains.DisableDeFiKey()) ||
             ConnectedChains.activeUpgradesByKey.count(ConnectedChains.DisablePBaaSCrossChainKey())))
        {
    ...
    int uniIdxOffset = 0;                  // leading notarizations with no root, omitted from the wire
    ...
    for (auto &oneNot : cnd.vtx)
    {
        ...
        bool absenceAllowed = false;
        oneNot.second.HasExpectedProofRoot(systemDef, &absenceAllowed);

        // only a leading definition, refunding, or pre-launch notarization may have no root
        if (proofRootsUni.size() || !absenceAllowed)
        {
            return state.Error(errorPrefix + "unexpected notarization with no proof root");
        }
        uniIdxOffset++;
    }
    ...
    oneParam.push_back(Pair("lastconfirmed", 0));
    ...
    catch (...)
    {
        if (logNotarization)
        {
            LogPrintf("%s: exception from getnotarizationdata\n", __func__);
        }
        notarizationResult = NullUniValue;
    }
    ...
    if (externalSystem.chainDefinition.IsPBaaSChain())
    {
        ...
        if (validIndexesUni.isArray() && validIndexesUni.size())
        {
            ...
            {
                int idx = uni_get_int(validIndexesUni[i], -1);
                if (idx >= 0 && idx < (int)proofRootsUni.size())
                {
                    validIndexSet.insert(idx + uniIdxOffset);
                }
            }
            ...
            for (int i = 1; i < cnd.vtx.size(); i++)
            {
                ...
                uint32_t nCount = unchallengedForks[j].size() ? k - 1 : 0;
                if (forceReset)
                {
                    nCount = 0;
                    for (int l = 1; l < k; l++)
                    {
                        auto it = mapBlockIndex.find(txes[unchallengedForks[j][l]].second);
                        if (it != mapBlockIndex.end() &&
                            (mapBlockIndex[txes[unchallengedForks[j][l]].second]->nTime < resetTime ||
                             mapBlockIndex[txes[unchallengedForks[j][l]].second]->GetHeight() < resetHeight))
                        {
                            continue;
                        }
                        nCount++;
                ...
                uint32_t adjustedNotarizationModulo = CPBaaSNotarization::GetAdjustedNotarizationModulo(ConnectedChains.ThisChain().blockNotarizationModulo,
                                                                                  std::max((uint32_t)pConfirmedNotarizationIndex->GetHeight(), resetHeight),
                                                                                  pCurNotarizationIndex->GetHeight(),
                                                                                  nCount);

                if (((cnd.vtx[unchallengedForks[j][k]].second.proofRoots.count(ASSETCHAINS_CHAINID) ?
                        cnd.vtx[unchallengedForks[j][k]].second.proofRoots[ASSETCHAINS_CHAINID].rootHeight : 0) / adjustedNotarizationModulo) <=
                    ((cnd.vtx[unchallengedForks[j][k - 1]].second.proofRoots.count(ASSETCHAINS_CHAINID) ?
                        cnd.vtx[unchallengedForks[j][k - 1]].second.proofRoots[ASSETCHAINS_CHAINID].rootHeight : 0) / adjustedNotarizationModulo))
                {
                    validIndexSet.erase(unchallengedForks[j][k]);
                }
    ...
    if (externalSystem.chainDefinition.IsPBaaSChain() && cnd.vtx[notaryIdx].second.proofRoots.count(SystemID))
    {
        ...
        for (int j = 0; j < challengeRequests.size(); j++)
        {
            ...
            {
                LogPrintf("%s: cross-chain API error: %s\n", __func__, find_value(challengeRequests[j], "error").write().c_str());
            }
        }
...

    {
        ...
        bool haveFork = cnd.forks.size() && cnd.bestChain >= 0 && cnd.forks[cnd.bestChain].size();
        uint32_t nCount = haveFork ? cnd.forks[cnd.bestChain].size() - 1 : 0;
        if (forceReset && haveFork)
        {
            nCount = 0;
            for (int l = 1; l < cnd.forks[cnd.bestChain].size(); l++)
            {
                auto it = mapBlockIndex.find(txes[cnd.forks[cnd.bestChain][l]].second);
                if (it != mapBlockIndex.end() &&
                    (mapBlockIndex[txes[cnd.forks[cnd.bestChain][l]].second]->nTime < resetTime ||
                     mapBlockIndex[txes[cnd.forks[cnd.bestChain][l]].second]->GetHeight() < resetHeight))
                {
                    continue;
                }
                nCount++;
        ...
        if ((height / adjustedNotarizationModulo) <=
            ((priorNotarization.proofRoots.count(ASSETCHAINS_CHAINID) ?
                priorNotarization.proofRoots.find(ASSETCHAINS_CHAINID)->second.rootHeight : 0) / adjustedNotarizationModulo))
        {
        ...
        chainActive.Contains(blockIt->second))
    {
                                      ...
                                      finalizationTypeKey == oneOF.ObjectFinalizationRejectedKey()))))
        {
            retVal.push_back(std::make_tuple(blockIt->second->GetHeight(),
                                             COutPoint(notarizationTx.GetHash(), (uint32_t)i),
                                             notarizationTx,
                                                      ...
                                                      const std::vector<std::pair<CTransaction, uint256>> &txAndBlockVec) const
{
    ...
    uint160 currencyID = vtx[0].second.currencyID;
    CCurrencyDefinition externalSystem = ConnectedChains.GetCachedCurrency(currencyID);
    ...
    if (externalSystem.IsPBaaSChain())
    {
              ...
              *forks[bestChain].rbegin() == (vtx.size() - 1)))
        {
            ...
            int nIdx;
            auto curIt = vtx[*forks[bestChain].rbegin()].second.proofRoots.find(currencyID);
            if (curIt == vtx[*forks[bestChain].rbegin()].second.proofRoots.end())
            {
                LogPrintf("%s: invalid best notarization for: %s\n", __func__, ConnectedChains.GetFriendlyCurrencyName(currencyID).c_str());
                return -1;
            }
            for (nIdx = *forks[bestChain].rbegin() + 1; nIdx < vtx.size(); nIdx++)
            {
                auto checkIt = vtx[nIdx].second.proofRoots.find(currencyID);
                if (checkIt == vtx[nIdx].second.proofRoots.end())
                {
                    continue;
                }
                if (CChainPower::ExpandCompactPower(checkIt->second.compactPower) >
                      CChainPower::ExpandCompactPower(curIt->second.compactPower))
                {
                    break;
                }
            }
    ...
    else
    {
        ...
        // TODO AUTONOTARIZATION: reenable full autonotarization after review and confirmation of security without witnesses
        else if (forks.size() &&
                 (forks[0].size() >= std::max(minNotaryConfirms, 1) ||
                  IsAfterBridgeCleanupWindowStarts(chainActive.LastTip()->nTime) ||
                  ConnectedChains.ThisChain().notarizationProtocol != CCurrencyDefinition::NOTARIZATION_AUTO))
        {
            bestFork = forks[0];
        }

        // recency was established for forks[bestChain]; re-check the fork actually selected
        if (bestFork.size())
        {
            auto selectedIt = mapBlockIndex.find(txAndBlockVec[bestFork.back()].second);
            if (selectedIt == mapBlockIndex.end() ||
                (height - selectedIt->second->GetHeight()) <= 0 ||
                (height - selectedIt->second->GetHeight()) > maxDepth)
            {
                LogPrint("notarization", "selected fork tip out of range\n");
                return -1;
            }
        }

        bool isNoRootOK = false;
        if (bestFork.size() < minNotaryConfirms ||
            (!vtx[lastConfirmed].second.HasExpectedProofRoot(externalSystem, &isNoRootOK) &&
             !isNoRootOK))
        {
        ...
        // a leading notarization with no root is omitted from the wire. every entry sent after that
        // must map 1:1 to a bestFork member, since the notary answers with indexes into this array
        int uniIdxOffset = isNoRootOK ? 1 : 0;
        ...
        for (auto forkIdxIt = bestFork.begin(); forkIdxIt != bestFork.end(); forkIdxIt++)
        {
            ...
            if (rootIt == oneNot.second.proofRoots.end())
            {
                // never send an invalid proof root, which makes the notary reject the entire request.
                // truncate here instead, leaving the valid prefix confirmable.
                // do not "continue" here, which would shift every later index
                LogPrint("notarization", "notarization with no proof root in fork, truncating\n");
                break;
            }
            proofRootsUni.push_back(rootIt->second.ToUniValue());
        }
        ...
        bool notaryIdxValid = false;
        for (int i = 0; i < proofRootArr.size(); i++)
        {
            if (uni_get_int(proofRootArr[i], -1) == notaryIdx)
            {
                notaryIdxValid = true;
                break;
            }
        ...
        if (!notaryIdxValid)
        {
            LogPrint("notarization", "index returned by notary is not in its own valid set\n");
            return -1;
        }

        int32_t forkIdx = notaryIdx + uniIdxOffset;
        if (notaryIdx >= (int32_t)proofRootsUni.size() ||
            forkIdx < 0 ||
            forkIdx >= (int32_t)bestFork.size())
        {
            LogPrint("notarization", "invalid index returned from notary\n");
            return -1;
        }
        // the finalization precheck requires minNotaryConfirms notarizations above the
        // one confirmed; never select a candidate without that headroom — an earlier
        // entry on the same fork is transitively approved by the notary's best/valid set
        int32_t maxConfirmIdx = (int32_t)bestFork.size() - 1 - minNotaryConfirms;
        if (maxConfirmIdx < 0)
        {
            LogPrint("notarization", "insufficient notarizations above candidate to confirm\n");
            return -1;
        }
        forkIdx = std::min(forkIdx, maxConfirmIdx);

        // like the PBaaS-chain path above, walk deeper until the candidate satisfies
        // the precheck's block depth requirement
        while (forkIdx >= 0)
        {
            auto depthIt = mapBlockIndex.find(txAndBlockVec[bestFork[forkIdx]].second);
            if (depthIt == mapBlockIndex.end())
            {
                LogPrint("notarization", "invalid block hash in notarization data\n");
                return -1;
            }
            if ((height - depthIt->second->GetHeight()) >= minBlockConfirms)
            {
                break;
            }
            forkIdx--;
                                                      ...
                                                      uint32_t nHeight)
{
    ...
    {
        auto &oneNot = cnd.vtx[*forkIdxIt].second;
        auto rootIt = oneNot.proofRoots.find(SystemID);

        if (rootIt != oneNot.proofRoots.end())
        {
            proofRootsUni.push_back(rootIt->second.ToUniValue());
            continue;
        }
        ...
        bool absenceAllowed = false;
        oneNot.HasExpectedProofRoot(externalSystem.chainDefinition, &absenceAllowed);
    ...
    {
        if (oneIndex >= uniIdxOffset)
        {
            disagreements.insert(oneIndex);
        }
    }

    // pre-accept only those leading best-fork positions that were omitted from the wire
    while (bestForkIdx < bestFork.size() && bestFork[bestForkIdx] < uniIdxOffset)
    {
        proofRootArr.push_back(bestForkIdx);
        bestForkIdx++;
    }

    for (int i = 0; i < rawProofRootArr.size(); i++)
    {
        int idx = uni_get_int(rawProofRootArr[i], -1);
        if (idx >= 0 && idx < (int)proofRootsUni.size())
        {
            disagreements.erase(idx + uniIdxOffset);

            if (bestForkIdx < bestFork.size() && (idx + uniIdxOffset) == bestFork[bestForkIdx])
            {
                proofRootArr.push_back(bestForkIdx);
                bestForkIdx++;
            }
                ...
                mine.size())
            {
                ...
                if (mine.size())
                {
                    ...
                    if (mergedEvidence.evidence.chainObjects.size())
                    {
                        ...
                        {
                            rejectOf.evidenceOutputs.push_back(txBuilder.mtx.vout.size() + loop);
                        }
    ...
    while (idx > 0)
    {
        ...
        // TODO AUTONOTARIZATION: remove time-based disablement after review and confirmation of autonotarization
        if (!IsAfterBridgeCleanupWindowStarts(chainActive.LastTip()->nTime) &&
            confirmationResult != CNotaryEvidence::EStates::STATE_CONFIRMED &&
            confirmationResult != CNotaryEvidence::EStates::STATE_REJECTED &&
                                                                      ...
                                                                      CValidationState &state)
{
    ...
    for (confirmingIdx = crosschainCND.vtx.size() - 1; confirmingIdx >= 0; confirmingIdx--)
    {
        ...
        uint32_t adjustedNotarizationModulo = externalSystem.chainDefinition.IsPBaaSChain() ? externalSystem.chainDefinition.blockNotarizationModulo : CCurrencyDefinition::BLOCK_NOTARIZATION_MODULO;
    ...
    bool submit = notaryRevokeID.IsNull() &&
                  (GetBoolArg("-alwayssubmitnotarizations", false) ||
                   ...
                   (externalSystem.chainDefinition.proofProtocol != CCurrencyDefinition::PROOF_ETHNOTARIZATION &&
                    newConfirmedNotarization.proofRoots.count(systemID) &&
                    crosschainCND.vtx[crosschainCND.lastConfirmed].second.proofRoots.count(systemID) &&
    ...
    if (notaryRevokeID.IsNull() && !submit)
    {
        ...
        if (!submit && lastConfirmedNotarization.proofRoots.count(ASSETCHAINS_CHAINID))
        {
            ...
            {
                LogPrint("notarization", "Uncaught exception determining pending exports to external chain %s\n", uni_get_str(params[0]).c_str());
                return retVal;
            }
            ...
            if (amWitness && submit)
            {
                LogPrintf("Witness found %zu pending exports for %s\n", result.size(),
                ...
                printf("Witness found %zu pending exports for %s\n", result.size(),
                                                                    externalSystem.chainDefinition.parent == ASSETCHAINS_CHAINID ?
                                                                        externalSystem.chainDefinition.name.c_str() :
                                                                        (externalSystem.chainDefinition.name + "." + ConnectedChains.ThisChain().name).c_str());
            }
                                    ...
                                    int32_t *pNotarizationCount)
{
           ...
           tmpBlockIt->second->GetHeight() >= lastConfirmedHeight)
    {
        ...
        if (forceReset && (tmpBlockIt->second->GetHeight() < resetHeight || tmpBlockIt->second->nTime < resetTime))
        {
            continue;
        }
        if (pNotarizationCount)
        {
            (*pNotarizationCount)++;
        }
    }
...
bool VerifyAndCheckImportNotarization(const CTransaction &tx, int32_t outNum, const CPBaaSNotarization &currentNotarization, bool &isImportNotarization, CValidationState &state, uint32_t height)
{
    ...
    if (cciForNot.sourceSystemID != ASSETCHAINS_CHAINID && !(cciForNot.IsDefinitionImport() && eOutStart == -1))
    {
        if (eOutStart <= 0 || eOutStart >= tx.vout.size())
        {
            return state.Error("Invalid import evidence");
        }
        CNotaryEvidence partialTxEvidence(tx, eOutStart, eOutEnd, CNotaryEvidence::TYPE_IMPORT_PROOF);
        // this should only be present as an accepted notarization for the notary chain in any normal case
        // on an import transaction.
        //
        // ensure that the proof root accurately reflects the proof root from the last confirmed notarization
        // for the source system
        std::vector<CAddressIndexDbEntry> addressIndex;
        if (!partialTxEvidence.output.hash.IsNull() &&
            partialTxEvidence.state == CNotaryEvidence::STATE_CONFIRMED &&
            partialTxEvidence.type == CNotaryEvidence::TYPE_IMPORT_PROOF &&
            currentNotarization.proofRoots.count(cciForNot.sourceSystemID))
        {
            bool confirmed = false;
            CTransaction confirmedNotTx;
            uint256 hashTxBlock;
            CPBaaSNotarization confirmedNotarization;

            if (GetAddressIndex(CCrossChainRPCData::GetConditionID(CObjectFinalization::ObjectFinalizationFinalizedKey(), partialTxEvidence.output.hash, partialTxEvidence.output.n), CScript::P2IDX, addressIndex) &&
                addressIndex.size())
            {
                std::vector<CAddressIndexDbEntry> confirmedIndex;

                // ensure that the address index entry is in the confirmed index
                CAddressIndexDbEntry keyEntry;
                for (auto &oneEntry : addressIndex)
                {
                    if (!oneEntry.first.spending)
                    {
                        keyEntry = oneEntry;
                        break;
                    }
                ...
                if (!GetAddressIndex(
                        CIndexID(CCrossChainRPCData::GetConditionID(CCrossChainRPCData::GetConditionID(cciForNot.sourceSystemID, CObjectFinalization::ObjectFinalizationNotarizationKey()),
                                                                    CObjectFinalization::ObjectFinalizationConfirmedKey())),
                                CScript::P2IDX,
                                confirmedIndex,
                                keyEntry.first.blockHeight,
                                keyEntry.first.blockHeight) ||
                    !confirmedIndex.size())
                {
                    return state.Error("Invalid notarization referenced by import evidence");
                }

                // get notarization from output and confirm proof root equivalence
                if (keyEntry.first.txhash.IsNull())
                {
                    return state.Error("Unrecognized notarization referenced by import evidence");
                }

                CTransaction finalizationTx;
                CObjectFinalization confirmedOF;
                for (auto &oneEntry : confirmedIndex)
                {
                    if (!oneEntry.first.spending && oneEntry.first.txhash == keyEntry.first.txhash && oneEntry.first.index == keyEntry.first.index)
                    {
                        if (myGetTransaction(keyEntry.first.txhash, finalizationTx, hashTxBlock, false) &&
                            keyEntry.first.index < finalizationTx.vout.size() &&
                            (confirmedOF = CObjectFinalization(finalizationTx.vout[keyEntry.first.index].scriptPubKey)).IsValid() &&
                            confirmedOF.GetOutputTransaction(finalizationTx, confirmedNotTx, hashTxBlock, false) &&
                            confirmedOF.output.n < confirmedNotTx.vout.size() &&
                            (confirmedNotarization = CPBaaSNotarization(confirmedNotTx.vout[confirmedOF.output.n].scriptPubKey)).IsValid() &&
                            confirmedNotarization.proofRoots.count(cciForNot.sourceSystemID) &&
                            currentNotarization.proofRoots.find(cciForNot.sourceSystemID)->second == confirmedNotarization.proofRoots.find(cciForNot.sourceSystemID)->second)
                        {
                            confirmed = true;
                            break;
                        }
            ...
            {
                // the only other way to confirm a notarization is to make sure it is in block #1
                if (myGetTransaction(partialTxEvidence.output.hash, confirmedNotTx, hashTxBlock, false) &&
                    !hashTxBlock.IsNull() &&
                    mapBlockIndex.count(hashTxBlock) &&
                    chainActive.Contains(mapBlockIndex.find(hashTxBlock)->second) &&
                    mapBlockIndex.find(hashTxBlock)->second->GetHeight() == 1 &&
                    partialTxEvidence.output.n < confirmedNotTx.vout.size() &&
                    (confirmedNotarization = CPBaaSNotarization(confirmedNotTx.vout[partialTxEvidence.output.n].scriptPubKey)).IsValid() &&
                    confirmedNotarization.proofRoots.count(cciForNot.sourceSystemID) &&
                    currentNotarization.proofRoots.find(cciForNot.sourceSystemID)->second == confirmedNotarization.proofRoots.find(cciForNot.sourceSystemID)->second)
                {
                    confirmed = true;
            ...
            if (!confirmed)
            {
                //if (LogAcceptCategory("notarization"))
                {
                    printf("%s: Cannot verify import notarization on tx: %s, outnum: %u, at height: %u\n", __func__, tx.GetHash().GetHex().c_str(), outNum, height);
                    LogPrintf("%s: Cannot verify import notarization on tx: %s, outnum: %u, at height: %u\n", __func__, tx.GetHash().GetHex().c_str(), outNum, height);
                }
                if (IsAfterBridgeCleanupWindowStarts(chainActive.ChainTimeAtOrBefore(height - 1)))
                {
                    return state.Error("Cannot verify import notarization");
                }
            }
        }
        else
        {
            return state.Error("Invalid import notarization");
        }
    }
    return true;
}
...
bool PreCheckAcceptedOrEarnedNotarization(const CTransaction &tx, int32_t outNum, CValidationState &state, uint32_t height)
{
    ...
    uint32_t curChainTipTime = chainActive.ChainTimeAtOrBefore(height - 1);
    if (IsAfterBridgeCleanupWindowStarts(curChainTipTime) &&
        currentNotarization.nVersion < CPBaaSNotarization::VERSION_PBAAS_MAINNET)
    {
        return state.Error("Invalid notarization version " + std::to_string(CPBaaSNotarization::VERSION_PBAAS_MAINNET));
    }
    ...
    for (int i = 0; i < tx.vout.size(); i++)
    {
        ...
        {
            return state.Error("Duplicate notarization output");
        }
    }
    ...
    if (currentNotarization.proofRoots.count(ASSETCHAINS_CHAINID))
    {
        ...
        {
            return state.Error("Incorrect proof root from future in notarization transaction");
        }
        CProofRoot correctRoot = CProofRoot::GetProofRoot(notarizationRoot.rootHeight);

        if (!correctRoot.IsValid())
        {
            return notarizationRoot.rootHeight > chainActive.Height() && chainActive.Height() < (height - 1) ? true : state.Error("Cannot verify proof root in notarization transaction");
        }
    ...
    if (p.evalCode == EVAL_ACCEPTEDNOTARIZATION &&
        curDef.IsValid() &&
        curDef.GetID() == curDef.SystemOrGatewayID() &&
        currentNotarization.currencyID != ASSETCHAINS_CHAINID &&
        !currentNotarization.HasExpectedProofRoot(curDef, &absenceAllowed) &&
        !absenceAllowed)
    {
        return state.Error("Accepted notarization must contain proof root of system being notarized unless it is a definition, refunding, or pre-launch notarization");
    }
    ...
    if (nHeight >= (height - 1))
    {
        ...
        {
            // if we are paused on cross-chain, return error until enabled
            if ((curChainTipTime < PBAAS_VERSION8_SOLUTION_TIME_START ||
                 !curDef.IsGateway() ||
                 p.evalCode == EVAL_ACCEPTEDNOTARIZATION) &&
                (ConnectedChains.activeUpgradesByKey.count(ConnectedChains.DisableDeFiKey()) ||
                 ConnectedChains.activeUpgradesByKey.count(ConnectedChains.DisablePBaaSCrossChainKey())))
            {
                return state.Error("Cross-chain functions temporarily disabled by notification oracle " + PBAAS_DEFAULT_NOTIFICATION_ORACLE);
            }
            ...
            if (p.evalCode == EVAL_ACCEPTEDNOTARIZATION)
            {
                ...
                if (!skipEnhancedCheck && height != 1 && !currentNotarization.IsPreLaunch())
                {
                    if (!VerifyAndCheckImportNotarization(tx, outNum, currentNotarization, isImportNotarization, state, height))
                    {
                        // state is set
                        return false;
                    }
            ...
            else
            {
                ...
                if (ConnectedChains.activeUpgradesByKey.count(ConnectedChains.DisableEarnedNotarizationKey()))
                {
                    return state.Error("Earned notarizations temporarily disabled by notification oracle " + PBAAS_DEFAULT_NOTIFICATION_ORACLE);
                }
                     ...
                     curDef.SystemOrGatewayID() != ASSETCHAINS_CHAINID)
                {
                    ...
                    if (!currentNotarization.IsBlockOneNotarization() && !currentNotarization.IsDefinitionNotarization())
                    {
                        ...
                        int64_t blockModulo = CPBaaSNotarization::GetAdjustedNotarizationModulo(ConnectedChains.ThisChain().blockNotarizationModulo,
                                                                                                std::max(std::get<0>(lastConfirmedNotarization), resetHeight),
                                                                                                height,
                                                                                                notarizationCount);
                        ...
                        if (additionalEvidenceRequired)
                        {
                            ...
                            {
                                if (priorNotarization.proofRoots[currentNotarization.currencyID] !=
                                    currentNotarization.proofRoots[currentNotarization.currencyID])
                                {
                                    return state.Error("Same height " +
                                                       std::to_string(currentNotarization.proofRoots[currentNotarization.currencyID].rootHeight) +
                                                       " different proofroot than prior notarization");
                                }
        ...
        else
        {
            ...
            else
            {
                     ...
                     curDef.SystemOrGatewayID() != ASSETCHAINS_CHAINID)
                {
                    ...
                    // TODO AUTONOTARIZATION: undo the time-based disablement after review and confirmation of return to autonotarization
                    bool signatureRequired = IsAfterBridgeCleanupWindowStarts(chainActive.LastTip()->nTime) ||
                                             !curDef.IsPBaaSChain() ||
                    ...
                    if (std::get<3>(priorNotarizationInfo).IsValid() &&
                        std::get<3>(priorNotarizationInfo).proofRoots.count(curDef.SystemOrGatewayID()) &&
                        currentNotarization.proofRoots.count(curDef.SystemOrGatewayID()) &&
                        std::get<3>(priorNotarizationInfo).proofRoots[curDef.SystemOrGatewayID()].rootHeight >= currentNotarization.proofRoots[curDef.SystemOrGatewayID()].rootHeight)
                    {
                        printf("%s: Must have some progression in the off-chain proof root of an accepted notarization that is not from an import for %s at height %u\n", __func__, curDef.name.c_str(), height);
                        LogPrintf("%s: Must have some progression in the off-chain proof root of an accepted notarization that is not from an import for %s at height %u\n", __func__, curDef.name.c_str(), height);
                        if (IsAfterBridgeCleanupWindowStarts(chainActive.ChainTimeAtOrBefore(height - 1)))
                        {
                            // imports are implicitly rejected above by the required state root progression anyhow
                            return state.Error("Must have some progression in the off-chain proof root of an accepted notarization that is not from an import");
                        }
                    }
                    ...
                    if (additionalEvidenceRequired && curDef.IsPBaaSChain())
                    {
                        ...
                        if (!IsValidPrimaryChainEvidence(curDef,
                                                         ...
                                                         0, // this is an accepted notarization, and as such, may not derive from an earned notarization. we will not enforce modulo by count here
...
CObjectFinalization::GetFinalizationEvidence(const CTransaction &thisTx, int32_t outNum, CValidationState &state, CTransaction *pOutputTx) const
{
    ...
    if (evidenceOutputs.size() || evidenceInputs.size())
    {
        ...
        for (int i = 0; i < evidenceInputs.size(); i++)
        {
            ...
            if (thisTx.vin.size() <= oneIn)
            {
                state.Error(std::string(__func__) + ": indexing error for finalization evidence input");
                LogPrint("notarization", "%s: indexing error for finalization evidence input\n", __func__);
                return emptyVec;
            }
            ...
            if (priorOutputTx.vout.size() <= thisTx.vin[oneIn].prevout.n)
            {
                state.Error(std::string(__func__) + ": evidence input references output beyond transaction");
                LogPrint("notarization", "%s: evidence input references output beyond transaction\n", __func__);
                return emptyVec;
            }
...
bool PreCheckNotaryEvidence(const CTransaction &tx, int32_t outNum, CValidationState &state, uint32_t height)
{
    ...
    if (currentEvidence.type == currentEvidence.TYPE_MULTIPART_DATA)
    {
        ...
        if (((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.type == CEvidenceData::TYPE_MULTIPART_DATA)
        {
            ...
            int64_t multiStart = ((int64_t)outNum - (int64_t)((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.index);
            if (multiStart != outNum &&
                multiStart >= 0)
            {
                if (tx.vout[outNum - 1].scriptPubKey.IsPayToCryptoCondition(multiP) &&
                    multiP.IsValid() &&
                    multiP.evalCode == EVAL_NOTARY_EVIDENCE &&
                    multiP.vData.size() &&
                    (multiEvidence = CNotaryEvidence(multiP.vData[0])).IsValid() &&
                    multiEvidence.type == multiEvidence.TYPE_MULTIPART_DATA &&
                    ...
                    ((CChainObject<CEvidenceData> *)(multiEvidence.evidence.chainObjects[0]))->object.md.index ==
                        (((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.index - 1) &&
                    (((CChainObject<CEvidenceData> *)(multiEvidence.evidence.chainObjects[0]))->object.md.totalLength == ((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.totalLength) &&
                    ((CChainObject<CEvidenceData> *)(multiEvidence.evidence.chainObjects[0]))->object.md.start + ((CChainObject<CEvidenceData> *)(multiEvidence.evidence.chainObjects[0]))->object.dataVec.size() ==
                        ((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.start &&
                    ((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.start < ((CChainObject<CEvidenceData> *)(currentEvidence.evidence.chainObjects[0]))->object.md.totalLength)
                {
                    return true;
                }
                else
    ...
    // now verify that the evidence we have passes a structural early filter
    // the referenced notarization must be retrievable, must be a valid earned/accepted notarization,
    // and its currency must resolve — with all substantive verification at the point of use.
...
bool PreCheckFinalizeNotarization(const CTransaction &tx, int32_t outNum, CValidationState &state, uint32_t height)
{
    ...
    if (currentFinalization.currencyID != notarization.currencyID)
    {
        return state.Error("Finalization for notarization has incorrect currency");
    }
    ...
    if (currentFinalization.IsConfirmed() || currentFinalization.IsRejected())
    {
        ...
        {
            // rule 1: any prior of opposite polarity is fatal, whether or not we spend anything
            for (auto &oneFinalization : priorFinalizations)
            {
                ...
                if (std::get<3>(oneFinalization).IsConfirmed() != currentFinalization.IsConfirmed())
                {
                    return state.Error(std::string("already ") +
                        (std::get<3>(oneFinalization).IsConfirmed() ? "confirmed" : "rejected"));
                }
            }

            // rule 2: a same-polarity prior is only OK if we are spending it
            std::set<CUTXORef> spent;
            for (auto &oneIn : tx.vin)
            {
                spent.insert(CUTXORef(oneIn.prevout));
            }
            for (auto &oneFinalization : priorFinalizations)
            {
                if (!spent.count(std::get<1>(oneFinalization)))
                {
                    return state.Error("already finalized");
                }
            }
            return true;
        }
        ...
        if (currentFinalization.IsConfirmed())
        {
            ...
            if (p.evalCode == EVAL_EARNEDNOTARIZATION)
            {
                ...
                // TODO AUTONOTARIZATION: undo time-based disablement after review and confirmation of return to autonotarization
                if (signatureState != CNotaryEvidence::STATE_CONFIRMED &&
                    (IsAfterBridgeCleanupWindowStarts(chainActive.ChainTimeAtOrBefore(height - 1)) ||
            ...
            else
            {
                      ...
                      notaryCurrencyDef.launchSystemID == ASSETCHAINS_CHAINID))
                {
                   if (notaryCurrencyDef.SystemOrGatewayID() != ASSETCHAINS_CHAINID &&
                    ...
                    // TODO AUTONOTARIZATION: remove time-based disablement (!IsAfterBridgeCleanupWindowStarts) after review and
                    // confirmation of return to autonotarization
                    if (signatureState != CNotaryEvidence::STATE_CONFIRMED &&
                        (IsAfterBridgeCleanupWindowStarts(chainActive.ChainTimeAtOrBefore(height - 1)) ||
                         !confirmNeedsEvidence ||
                         pNotaryCurrency->proofProtocol != pNotaryCurrency->PROOF_PBAASMMR ||
            ...
            if (!invalidates)
            {
                ...
                if (p.evalCode == EVAL_EARNEDNOTARIZATION)
                {
                    ...
                    if (!evidenceMap.count(ASSETCHAINS_CHAINID) ||
                        !evidenceMap[ASSETCHAINS_CHAINID].size() ||
                ...
                else
                {
                    ...
                    if ((notaryCurrencyDef.IsPBaaSChain() || notaryCurrencyDef.IsGateway()) &&
...
bool ValidateFinalizeNotarization(struct CCcontract_info *cp, Eval* eval, const CTransaction &tx, uint32_t nIn, bool fulfilled)
{
    ...
    // if we are spending a confirmed finalization, then one of the following must be true:
    // 1) we are being spent by an accepted notarization and have the same confirmation on an output just after a pending output,
    // 2) this finalization is the first finalization for the new notarization being confirmed and is spending a confirmed finalization for
    //    an earlier confirmed notarization
    if (oldFinalization.IsConfirmed())
    {
        int priorPendingFinalization = -1, notarizationOut = -1;

        uint256 oldBlockHash;
        CTransaction oldNotarizationTx;
        if (oldHeight > 0 && oldHeight <= chainActive.Height())
        {
            oldBlockHash = chainActive[oldHeight]->GetBlockHash();
        }
        if (!oldFinalization.GetOutputTransaction(sourceTx, oldNotarizationTx, oldBlockHash, false))
        {
            return eval->Error("Unable to get transaction for confirmed output for finalization on tx: " + sourceTx.GetHash().GetHex() + ", output #" + std::to_string(tx.vin[nIn].prevout.n));
        }

        auto oldBlockIt = mapBlockIndex.find(oldBlockHash);

        if (oldBlockIt == mapBlockIndex.end() || !chainActive.Contains(oldBlockIt->second))
        {
            return eval->Error("Unable to find block for transaction with finalization on tx: " + sourceTx.GetHash().GetHex() + ", output #" + std::to_string(tx.vin[nIn].prevout.n));
        }
            ...
            if (oneOut.scriptPubKey.IsPayToCryptoCondition(p) &&
                ...
                ((p.evalCode == EVAL_FINALIZE_NOTARIZATION &&
                 ...
                 (p.evalCode == EVAL_ACCEPTEDNOTARIZATION &&
                  ...
                  n.currencyID == oldFinalization.currencyID)))
            {
                if (p.evalCode == EVAL_FINALIZE_NOTARIZATION &&
                    oneOF.IsConfirmed() &&
                    ((priorPendingFinalization != -1 && notarizationOut != -1 && oneOF.output == oldFinalization.output) ||
                      oneOF.output != oldFinalization.output))
                {
                    if (oneOF.output != oldFinalization.output)
                    {
                        // ensure that the new confirmed notarization is after the last on-chain, finalization validity will be ensured by its precheck
                        uint256 newBlockHash;
                        CTransaction newNotarizationTx;
                        if (oneOF.GetOutputTransaction(tx, newNotarizationTx, newBlockHash, false))
                        {
                            auto newBlockIt = mapBlockIndex.find(newBlockHash);

                            if (newBlockIt != mapBlockIndex.end() &&
                                chainActive.Contains(newBlockIt->second) &&
                                oldBlockIt->second->GetHeight() < newBlockIt->second->GetHeight())
                            {
                                return true;
                            }
                        }
        ...
        if (LogAcceptCategory("notarization") && LogAcceptCategory("verbose"))
        {
            ...
            printf("%s: Invalid spend of confirmed finalization on transaction:\n%s\n", __func__, tx.GetHash().GetHex().c_str());
        }
...
bool CObjectFinalization::GetOutputTransaction(const CTransaction &initialTx, CTransaction &tx, uint256 &blockHash, bool checkMempool) const
{
    if (output.hash.IsNull())
    {
        if (output.n >= initialTx.vout.size())
        {
            return false;
        }
        tx = initialTx;
